...
#ifndef TIMERS_SPINTERFACE_H
#define TIMERS_SPINTERFACE_H
...
enum CharacterFormat : std::uint8_t
{
    lsb_8bit = 0x00,
    lsb_7bit = 0x10,
    msb_8bit = 0x20,
...
template<typename data_type,
         ...
         Mode mode = master_3pin,
         CharacterFormat format = msb_8bit,
         ClockMode clock_mode = active_high_leading_edge,
         ClockSource clock_source = smclk,
...
struct USCISPI
{
    ...
    {
        while(!(*reinterpret_cast<volatile std::uint8_t*>(interrupt_flag_register) & rx_interrupt_bit)); //TODO non-blocking
